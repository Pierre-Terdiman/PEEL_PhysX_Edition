...
namespace
{
	...
	{
		public:
								JointListBoxes() : mJointsFromActor0(null), mJointsFromActor1(null)	{}
								~JointListBoxes()	{}
				...
				IceListBox*		mJointsFromActor0;
				IceListBox*		mJointsFromActor1;
	};
...
PintJointHandle JointListBoxes::GetSelectedJoint(udword id, Pint_Joint* joint_api, PintActorHandle actor, PR& pose)
{
	...
	class GearJointWindow : public IceWindow
	{
		...
		virtual					~GearJointWindow();
				...
				IceEditBox*		mEditBox_NbTeeth0;
				IceEditBox*		mEditBox_NbTeeth1;
...
bool GearJointWindow::ListHinges(Pint& pint, PintActorHandle actor, udword id)
{
	...
	class RackJointWindow : public IceWindow
	{
		...
		virtual					~RackJointWindow();
				...
				IceEditBox*		mEditBox_NbTeethPinion;
				IceEditBox*		mEditBox_NbTeethRack;
				IceEditBox*		mEditBox_RackLength;
				IceCheckBox*	mCheckBox_SelectShape;
//				IceComboBox*	mComboBox_ErrorSign;
				JointListBoxes	mListBoxes;
				AABB			mRackBounds;
	};
...
RackJointWindow::RackJointWindow(const WindowDesc& desc) :
	IceWindow				(desc),
	mEditBox_NbTeethPinion	(null),
	mEditBox_NbTeethRack	(null),
	mEditBox_RackLength		(null),
	mCheckBox_SelectShape	(null)
//	mComboBox_ErrorSign		(null)
{
...
//	ResetButtonNames(mActor0==null, mActor1==null);
	...
	if(SelectedIndex==IJT_SPHERICAL)
	{
...
{
	if(mJointType)
		mJointType->Select(IJT_SPHERICAL);
	OnComboBoxChange();
...
void ToolCreateJointBase::CreateToolComboBox(PintGUIHelper& helper, IceWidget* parent, Widgets& owner, sdword x, sdword y)
{
	...
	mJointType->Add("Spherical");		// IJT_SPHERICAL
...
static void SnapPos(EditPosWindow* mEditPos0, EditPosWindow* mEditPos1, const PR& Pose0, const PR& Pose1)
{
...
static void Snap0(Pint& pint, PintActorHandle actor0, PintActorHandle actor1, EditPosWindow* edit_pos0, EditPosWindow* edit_pos1, Quat& edit_rot0, const Quat& edit_rot1)
{
	ASSERT(actor0);
	ASSERT(actor1);
	ASSERT(edit_pos0);
	...
	const PR Pose0 = pint.GetWorldTransform(actor0);
	const PR Pose1 = pint.GetWorldTransform(actor1);
...
static void Snap1(Pint& pint, PintActorHandle actor0, PintActorHandle actor1, EditPosWindow* edit_pos0, EditPosWindow* edit_pos1, const Quat& edit_rot0, Quat& edit_rot1)
{
	ASSERT(actor0);
	ASSERT(actor1);
	ASSERT(edit_pos0);
...
void ToolCreateJoint::RenderCallback(Pint& pint, udword pint_index)
{
...
void ToolCreateJoint::CreateUI(PintGUIHelper& helper, IceWidget* parent, Widgets& owner)
{
	...
	{
		mButton0 = GUI_CreatePushButton(parent, 0, x, y, 200, 20, gButtonLabel0, &owner, Callbacks::SelectActor, this, null);
		y += YStep;

		mButton1 = GUI_CreatePushButton(parent, 0, x, y, 200, 20, gButtonLabel1, &owner, Callbacks::SelectActor, this, null);
		y += YStep;
...

	{
		...
		const sdword NameY = 4+YStep+2+8;
		helper.CreateLabel(parent, NameX, NameY, 40, 20, "Name:", &owner);
		mEditBoxName = helper.CreateEditBox(parent, 0, NameX+40, NameY, 200, 20, "JointName", &owner, EDITBOX_TEXT, null);
	}

	y += YStep;
	CreateToolComboBox(helper, parent, owner, x, y);
//	y += YStep;
...
void ToolCreateChain::RenderCallback(Pint& pint, udword pint_index)
{
	if(mPint==&pint)
	{
		const udword NbLinkActors = mLinkActors.GetNbLinkActors();
