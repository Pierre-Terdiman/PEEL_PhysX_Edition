...
static const Point gDefaultGravity(0.0f, -9.81f, 0.0f);
...
void SetFocusShape(PintShapeHandle h)
{
	gPintRender->mFocusShape = h ? Editor_GetNativeHandle(h) : null;
}
...
//	gMyControlInterface.RenderCallback();
	...
	if(gRenderEnabled && !gMenuIsVisible)
	{
		GetControlInterface()->RenderCallback();
...
//	GLRenderHelpers::DrawFrame(Point(0.0f, 0.0f, 0.0f));
...
//			gTexter.print(x, y, HelpTextScale, "Use the mouse and right mouse button for picking");						y -= HelpTextScale;
		...
		if(gCandidateTest)
		{
			...
			GLRenderHelpers::DrawRectangle(XOffset, 1.0f-XOffset, ystart, ystart - PanelSize, Point(1.0f, 0.5f, 0.2f), Point(0.0f, 0.0f, 0.0f), 0.5f, gScreenWidth, gScreenHeight, true, GLRenderHelpers::SQT_DISABLED);
...
//		WD.mStyle	= WSTYLE_STATIC_EDGES;
		gEditBox_Gravity = ICE_NEW(EditPosWindow)(WD);
...
//		IceEditBox* tmp = gGUIHelper.CreateEditBox(tab, MAIN_GUI_DUMMY, 296, saved, 180, 140, "=== Raytracing test ===", gMainGUI, EDITBOX_TEXT, null);
		IceEditBox* tmp = gGUIHelper.CreateEditBox(tab, MAIN_GUI_DUMMY, x+296-310, saved, 180, 140, "=== Raytracing test ===", gMainGUI, EDITBOX_TEXT, null, gTooltip_RaytracingTest);
//		tmp->AddToolTip(gTooltip_RaytracingTest);
		tmp->SetReadOnly(true);
	}
	tab->SetVisible(false);
...
//	y += YStep;
	...
	{
		sdword y2 = 10;
		gGUIHelper.CreateButton(tab, COLOR_PICKER_WIRE_COLOR, 300, 50, 130, 20, "Change wire color", gMainGUI, gButtonCallback, "Select: wire color"/*tab_control*/);
		y2 += 30;
	}
...
//				y += 30;
		...
		}
		{
			...
			{
				IceWindow* cp = CreateColorPicker2(0, GUI_HIDDEN_POS_X, GUI_HIDDEN_POS_Y, &gColorPickerCallback);
			...
			{
				EditActorWindow* ui = CreateActorEditGUI(0, GUI_HIDDEN_POS_X, GUI_HIDDEN_POS_Y);
				ui->SetVisible(false);
				ui->SetPosition(4, 4);
			}
			{
				CompoundCreateWindow* ui = CreateCompoundCreateGUI(0, GUI_HIDDEN_POS_X, GUI_HIDDEN_POS_Y);
				ui->SetVisible(false);
				ui->SetPosition(4, 4);
			}
			{
				EditJointWindow* ui = CreateJointEditGUI(0, GUI_HIDDEN_POS_X, GUI_HIDDEN_POS_Y);
				ui->SetVisible(false);
				ui->SetPosition(4, 4);
			}	
		}
