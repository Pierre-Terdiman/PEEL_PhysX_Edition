...
#include ".\PhysicsData.h"
	...
	struct MergeParams
	{
		MergeParams(const char* name=null, float mass=-1.0f, const PR* pose=null) : mName(name), mMass(mass), mPose(pose)
		{
		}
...
/*		virtual	bool				ResetVehicleData(PintVehicleHandle vehicle);
	...
	class EditorPlugin : public Pint
	{
...
//		virtual	void						SetRCArticulatedMotor(PintActorHandle object, const PINT_ARTICULATED_MOTOR_CREATE& motor);
...
#endif
		virtual	bool						GetHingeInfo(PintJointHandle handle, PintHingeInfo& info)							override;
		virtual	bool						GetD6Info(PintJointHandle handle, PintD6Info& info)									override;
		...
		//~Pint
				...
				PintActorHandle				CreateSingle(udword nb, const PintActorHandle* handles);
		...
		inline_	const ActorData*			GetEditorActor(udword i)		const	{ return (const ActorData*)mEditorActors.GetEntry(i);			}
		...
		inline_	const JointData*			GetEditorJoint(udword i)		const	{ return (const JointData*)mEditorJoints.GetEntry(i);			}
