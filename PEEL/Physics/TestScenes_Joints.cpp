...
static float PrintTwistAngle(Pint& pint, GLFontRenderer& renderer, float y, float text_scale)
{
	...
	PintHingeInfo Info;
	if(!pint.GetHingeInfo(Handle, Info))
		return y;
	renderer.print(0.0f, y, text_scale, _F("Angle: %.5f\n", Info.mTwistAngle * RADTODEG));
	return y - text_scale;
}
...
static bool CreateJointedCubes(Pint& pint, const PintCaps& caps, const Point& static_pos, const Point& local_axis, float limit)
{
	...
	if(limit>0.0f)
	{
		Desc.mMinLimitAngle	= degToRad(-limit);
		Desc.mMaxLimitAngle	= degToRad(limit);
	}
...
static const char* gDesc_SphericalWeird = "Spherical weird";

START_TEST(SphericalWeird, CATEGORY_JOINTS, gDesc_SphericalWeird)
	...
	virtual bool	Setup(Pint& pint, const PintCaps& caps)
	{
		...
		const Point Pos0(0.0f, Radius, 0.0f);
		const Point Pos1(0.0f, Radius*3.0f, 0.0f);
//		const Point Pos1(Radius*2.0f, Radius, 0.0f);

		PintActorHandle Actor0 = CreateDynamicObject(pint, &SphereDesc, Pos0);
		PintActorHandle Actor1 = CreateDynamicObject(pint, &SphereDesc, Pos1);

		// Comment this line out to see the fucking weird difference. And then try to use the translate tool on the object for even more weirdness.
		pint.Update(1.0f/60.0f);

		PintJointHandle JointHandle = pint.CreateJoint(PINT_SPHERICAL_JOINT_CREATE(Actor0, Actor1, Point(0,0,0), Point(0,0,0)));
		(void)JointHandle;

		return true;
	}

END_TEST(SphericalWeird)

/*static const char* gDesc_SphericalSingularity = "Spherical singularity";

START_TEST(SphericalSingularity, CATEGORY_JOINTS, gDesc_SphericalSingularity)
...
//		const Point LinVel(0.0f, 0.0f, 1.0f);
		...
		PintJointHandle JointHandle = pint.CreateJoint(PINT_SPHERICAL_JOINT_CREATE(StaticObject, DynamicObject, Disp*0.5f, -Disp*0.5f));

		return true;
	}

END_TEST(SphericalSingularity)*/
...
START_TEST(HingeJoint, CATEGORY_JOINTS, gDesc_HingeJoint)
	...
	{
//		pint.AddLocalTorque(PintActorHandle(pint.mUserData), Point(0.0f, 0.0f, 1000.0f));
//		pint.SetLocalRot(PintActorHandle(pint.mUserData), Quat(Idt));
		return 0;
	}
...
START_TEST(HingeJoint, CATEGORY_JOINTS, gDesc_HingeJoint)
	...
	{
		TestBase::GetSceneParams(desc);
		desc.mCamera[0] = PintCameraPose(Point(4.36f, 20.73f, 4.06f), Point(-0.75f, -0.07f, -0.66f));
		SetDefEnv(desc, false);
	}
	...
	virtual bool	Setup(Pint& pint, const PintCaps& caps)
	{
		...
		const Point Pos(0.0f, 20.0f, 0.0f);
		...
		if(0)
		{
			...
			Desc.mLocalPivot1	= Point(-Extents.x, 0.0f, 0.0f);
		}
		else
		{
			Desc.mObject1		= null;
			Desc.mObject0		= h;
			Desc.mLocalPivot1	= Pos;
			Desc.mLocalPivot0	= Point(-Extents.x, 0.0f, 0.0f);
		}
...
START_TEST(CheckCollisionBetweenJointed, CATEGORY_JOINTS, gDesc_CheckCollisionBetweenJointed)
...
START_TEST(LimitedHingeJoint3, CATEGORY_JOINTS, gDesc_LimitedHingeJoint3)
...
/*
- hinge motor goes to sleep??
- picking / applying force doesn't work as expected on motorized revolute joint when the axis is at the origin. No similar issue with RCA.
- gear joint doesn't work when support is dynamic and we move it around.
- the teeth are overlapping eachother
- doesn't work with TGS
- body frame pos is not fixed and weird => because of com/convexes? check with boxes
- drop pivot pos from desc? always 0...
- drop explicit gear ratio from API
*/
...
//		return PrintTwistAngle(pint, renderer, y, text_scale);
		...
		{
			PintHingeInfo Info;
			if(!pint.GetHingeInfo(mHingeJoint0, Info))
				return y;
			Angle0 = Angle0b = Info.mTwistAngle;
		}
		...
		{
			PintHingeInfo Info;
			if(!pint.GetHingeInfo(mHingeJoint1, Info))
				return y;
			Angle1 = Angle1b = -Info.mTwistAngle;
		}
...
//		const char* Filename = FindPEELFile("TwoGears.zb2");
	...
	virtual bool	Setup(Pint& pint, const PintCaps& caps)
	{
		...
		if(SelectedIndex==1)
		{
			...
			ASSERT(mZB2Factory->mActors.GetNbEntries()==2);
			...
			const PINT_OBJECT_CREATE* Create0 = (const PINT_OBJECT_CREATE*)mZB2Factory->mActors.GetEntry(0);
			const PINT_OBJECT_CREATE* Create1 = (const PINT_OBJECT_CREATE*)mZB2Factory->mActors.GetEntry(1);
			...
			Box0.mLocalPos	= Create0->mShapes->mLocalPos;
			Box0.mLocalRot	= Create0->mShapes->mLocalRot;
			Box0.mMaterial	= Create0->mShapes->mMaterial;
			Box0.mRenderer	= Create0->mShapes->mRenderer;	// Keep the Lego triangle mesh renderer
			ASSERT(!Create0->mShapes->mNext);

			PINT_CONVEX_CREATE Box1(mCylinder2.mNbVerts, mCylinder2.mVerts);
			Box1.mLocalPos	= Create1->mShapes->mLocalPos;
			Box1.mLocalRot	= Create1->mShapes->mLocalRot;
			Box1.mMaterial	= Create1->mShapes->mMaterial;
			Box1.mRenderer	= Create1->mShapes->mRenderer;	// Keep the Lego triangle mesh renderer
			ASSERT(!Create1->mShapes->mNext);
...
/*							const Matrix3x3 M0 = pose0.mRot;
						...
						if(limits)
						{
							Desc.mMinLimitAngle = -HALFPI/2.0f;
							Desc.mMaxLimitAngle = HALFPI/2.0f;
						}
...
//					Rot.Identity();
	...
	virtual	float	DrawDebugText(Pint& pint, GLFontRenderer& renderer, float y, float text_scale)
	{
		if(mHingeJoint0)
		{
			...
			if(!pint.GetHingeInfo(mHingeJoint0, Info))
				return y;
			renderer.print(0.0f, y, text_scale, _F("Angle0: %.5f\n", Info.mTwistAngle * RADTODEG));	y -= text_scale;
		}
		return y;
	}
...
//		return CreateZB2Scene(pint, caps);
		...
		ASSERT(!mZB2Factory->mNbDisabledGroups);
		ASSERT(mZB2Factory->mActors.GetNbEntries()==3);
		ASSERT(!mZB2Factory->mJoints.GetNbEntries());

		const PINT_OBJECT_CREATE* Create0 = (const PINT_OBJECT_CREATE*)mZB2Factory->mActors.GetEntry(0);
		const PINT_OBJECT_CREATE* Create1 = (const PINT_OBJECT_CREATE*)mZB2Factory->mActors.GetEntry(1);
		const PINT_OBJECT_CREATE* Create2 = (const PINT_OBJECT_CREATE*)mZB2Factory->mActors.GetEntry(2);
		...
		Box0.mLocalRot	= Create0->mShapes->mLocalRot;
		Box0.mMaterial	= Create0->mShapes->mMaterial;
		Box0.mRenderer	= Create0->mShapes->mRenderer;
		ASSERT(!Create0->mShapes->mNext);
		...
		Box1.mLocalRot	= Create1->mShapes->mLocalRot;
		Box1.mMaterial	= Create1->mShapes->mMaterial;
		Box1.mRenderer	= Create1->mShapes->mRenderer;
		ASSERT(!Create1->mShapes->mNext);

		PINT_CONVEX_CREATE Box2(mCylinder.mNbVerts, mCylinder.mVerts);
		Box2.mLocalPos	= Create2->mShapes->mLocalPos;
		Box2.mLocalRot	= Create2->mShapes->mLocalRot;
		Box2.mMaterial	= Create2->mShapes->mMaterial;
		Box2.mRenderer	= Create2->mShapes->mRenderer;
		ASSERT(!Create2->mShapes->mNext);
...
{
	const PINT_SHAPE_CREATE* Shape0 = create->mShapes;
...
//		const float Radius = 0.81f;
	...
	virtual	void	GetSceneParams(PINT_WORLD_CREATE& desc)
	{
		...
		if(Filename)
		{
			...
			{
				ASSERT(!mZB2Factory->mNbDisabledGroups);
				ASSERT(mZB2Factory->mActors.GetNbEntries()==4);
				ASSERT(!mZB2Factory->mJoints.GetNbEntries());
...
//		SceneRot.Identity();
		...
		const PINT_OBJECT_CREATE* Create1 = (const PINT_OBJECT_CREATE*)mZB2Factory->mActors.GetEntry(1);
		const PINT_OBJECT_CREATE* Create2 = (const PINT_OBJECT_CREATE*)mZB2Factory->mActors.GetEntry(2);
		const PINT_OBJECT_CREATE* Create3 = (const PINT_OBJECT_CREATE*)mZB2Factory->mActors.GetEntry(3);
		...
		//{
			...
			Convex.mLocalPos	= Create1->mShapes->mLocalPos;
			Convex.mLocalRot	= Create1->mShapes->mLocalRot;
			Convex.mMaterial	= Create1->mShapes->mMaterial;
			Convex.mRenderer	= Create1->mShapes->mRenderer;
			ASSERT(!Create1->mShapes->mNext);

			Copy1.mShapes = &Convex;
			Copy1.mMass = 1.0f;
...
{
	PINT_BOX_CREATE Box0(extents);
	Box0.mLocalPos	= create->mShapes->mLocalPos;
	Box0.mLocalPos += center;
	Box0.mLocalRot	= create->mShapes->mLocalRot;
	Box0.mMaterial	= create->mShapes->mMaterial;
	Box0.mRenderer	= create->mShapes->mRenderer;
	ASSERT(!create->mShapes->mNext);
	...
	virtual	void	GetSceneParams(PINT_WORLD_CREATE& desc)
	{
		...
		if(Filename)
		{
			...
			if(mZB2Factory)
			{
				...
				for(udword i=0;i<3;i++)
					PreprocessMesh(mCenter[i], mExtents[i], (const PINT_OBJECT_CREATE*)mZB2Factory->mActors.GetEntry(i));
			}
		}
	}
...
{
			IceEditBox*		mEditBox_LimitAngle;
	public:
							ConfigurableLimitedHingeJoint() :
									mEditBox_LimitAngle(null)
															{												}
...
class JacobsLadder : public TestBase
{
	class LocalTestData : public TestData
	{
		public:
			...
			IceCheckBox*	mCheckBox_EnableMotor;
			IceCheckBox*	mCheckBox_TweakInertia;
			IceEditBox*		mEditBox_TargetVel;
			IceComboBox*	mComboBox_JointType;
//			IceSlider*		mSlider;
	public:
							JacobsLadder() :
								mCheckBox_EnableMotor	(null),
								mCheckBox_TweakInertia	(null),
								mEditBox_TargetVel		(null),
...
//			Desc.mMinLimitAngle		= SideY*HALFPI;
...

		{
			...
			pint.CreateJoint(Desc);
				Desc.mMinLimitAngle		= MIN_FLOAT;
				Desc.mMaxLimitAngle		= MAX_FLOAT;
	...
	#define NB_OBJECTS	6
		...
		if(1)
		{
			...
			Objects[0] = CreateObject(pint, &ObjectDesc, Mass, Object0Pos, null, 2);
			{
				...
				fjc.mMaxLimitAngle	= 0.0f*DEGTORAD;
//				fjc.mMaxLimitAngle	= -90.0f*DEGTORAD;
				fjc.mMinLimitAngle	= -270.0f*DEGTORAD;
...
START_TEST(LimitedPrismaticJoint2, CATEGORY_JOINTS, gDesc_LimitedPrismaticJoint2)
...
static const char* gDesc_HingeJointMotor = "Hinge joints motor test";
...
START_TEST(HingeJointMotorVsObstacle, CATEGORY_JOINTS, gDesc_HingeJointMotorVsObstacle)
...
static const char* gDesc_HingeJointMotorsZeroVelocity = ".";

START_TEST(HingeJointMotorsZeroVelocity, CATEGORY_JOINTS, gDesc_HingeJointMotorsZeroVelocity)
...
/*		PintActorHandle PrevObject = CreateDynamicObject(pint, &BoxDesc, Pos);
		{
			PINT_FIXED_JOINT_CREATE Desc;
			Desc.mObject0		= null;
			Desc.mObject1		= PrevObject;
			Desc.mLocalPivot1	= -Pos;
			PintJointHandle JointHandle = pint.CreateJoint(Desc);
			ASSERT(JointHandle);
		}*/
...
//				Desc.mLocalPivot1	= Point(0.0f, 0.0f, 0.0f);
		...
		else
		{
			...
			{
				PM.Random(256);
				const udword* List = PM.GetList();

			for(udword ii=0;ii<Nb;ii++)
			{
				udword i = List[ii];
	//			udword i = Nb-1-ii;
	//			udword i = ii;
	...
	virtual	udword		Update(Pint& pint, float dt)
	{
		if(0)
		{
			...
			{
				pint.SetAngularVelocity(Handles[i], Point(0.0f, 0.0f, 0.0f));
			}
		}
		return 0;
	}
...
END_TEST(HingeJointMotorsZeroVelocity)
...
class FixedJointsTorus : public TestBase
{
			...
			IceComboBox*	mComboBox_Preset;
			IceCheckBox*	mCheckBox_StressTest;
	public:
							FixedJointsTorus() :
								mEditBox_Desc	(null),
								mComboBox_Preset(null)
...
static const char* gDesc_ConfigurableSphericalChain = "(Configurable test) - Spherical chain. This tutorial demonstrates various ways to make \
...
static const char* gDesc_CaterpillarTrackOnBridge = "Caterpillar track on bridge. Benchmark scene for a lot of joints.";

START_TEST(CaterpillarTrackOnBridge, CATEGORY_WIP, gDesc_CaterpillarTrackOnBridge)
	...
	void	CreateScene(Pint& pint, const Point& pos)
	{
		...
		CreateCaterpillarTrack_OLD(pint, TO, &mHighFrictionMaterial, pos+Point(3.0f, 2.0f, 0.0f), mCylinder, GetRegisteredRenderers()[0], mCylinder2, GetRegisteredRenderers()[1]);
...
//START_TEST(LegoTechnicKart, CATEGORY_JOINTS, gDesc_LegoTechnicKart)
class LegoTechnicKart : public VehicleInput
{
	public:
							LegoTechnicKart()			{								}
...
//			printf("...CustomizeJoint\n");
	...
	{
		mCameraStyle = true;
		gZCB2_RenderScale = 0.35f;
		//gZCB2_RenderScale = 0.1f;
		...
		if(Filename)
		{
			...
			if(0)
			{
				...
				{
					PINT_OBJECT_CREATE* Create = reinterpret_cast<PINT_OBJECT_CREATE*>(mZB2Factory->mActors[i]);
					if(Create)
					{
						const udword NbShapes = Create->GetNbShapes();
						printf("Actor %d: (%d shapes) %s\n", i, NbShapes, Create->mName);
					}
				}
			}
...
//			printf("%d joints\n", objects.mNbJoints);

			{
				...
				{
					PintActorHandle CurrentActor = objects.mActors[i];
...
/*		Point CameraPos = mCameraTarget + Point(0.0f, 10.0f, 10.0f);
	...
	virtual udword	Update(Pint& pint, float dt)
	{
		...
		if(PD && PD->mChassis)
		{
			...
			mCamera.mUpOffset = 0.5f;
			mCamera.mDistToTarget = 8.0f;
