...
void UpdateSelectionItems()
{
	if(gMenu_Selection)
	{
		const bool HasSelection = GetDefaultControlInterface().mSelMan.HasSelection();
		const udword NbSelected = GetDefaultControlInterface().mSelMan.GetNbSelected();
		gMenu_Selection->SetEnabled(MENU_HIDE_SELECTED, HasSelection);
		gMenu_Selection->SetEnabled(MENU_SHOW_SELECTED, HasSelection);
		gMenu_Selection->SetEnabled(MENU_HIDE_UNSELECTED, HasSelection);
		gMenu_Selection->SetEnabled(MENU_DELETE_SELECTED, HasSelection);
		gMenu_Selection->SetEnabled(MENU_CANCEL_SELECTION, HasSelection);
		gMenu_Selection->SetEnabled(MENU_EDIT_SELECTED, NbSelected==1);
	}
}
...
void CreatePEELMenuBar(IceWindow* parent, Widgets& widgets)
{
	...
	IceMenu* Menu_File = ICE_NEW(IceMenu);
	{
		...
		Menu_File->Add("Load", MENU_LOAD);
		Menu_File->Add("Merge", MENU_MERGE);
