...
#undef FillMemory
	...
	#define ICE_SAFE_RELEASE(x)		if (x) { (x)->Release();		(x) = null; }

	//! Safe ICE-style release
	#define ICE_SAFE_DESTRUCT(x)	if (x) { (x)->SelfDestruct();	(x) = null; }
...
#endif
	#define	ICE_FREE(x)				if(x)	{ GetAllocator()->free(x, false); x = null;	}
