...
static float GetJointContactDistance(const EditableParams& params)
{
	...
	switch(desc.mType)
	{
		case PINT_JOINT_SPHERICAL:
		{
			...
			if(0)
			{
				printf("mObject0: %d\n", jc.mObject0);
				printf("mObject1: %d\n", jc.mObject1);
				printf("mLocalPivot0: %f %f %f\n", jc.mLocalPivot0.x, jc.mLocalPivot0.y, jc.mLocalPivot0.z);
				printf("mLocalPivot1: %f %f %f\n", jc.mLocalPivot1.x, jc.mLocalPivot1.y, jc.mLocalPivot1.z);		
			}
...
//			const float limitContactDistance = FLT_MAX;
			...
			const PxJointAngularLimitPair limit(jc.mMinLimitAngle, jc.mMaxLimitAngle, ContactDistance);
			const bool IsLimitedHinge = jc.mMinLimitAngle!=MIN_FLOAT || jc.mMaxLimitAngle!=MAX_FLOAT;
			NeedsExtendedLimits = IsLimitedHinge && (jc.mMinLimitAngle<=-PI || jc.mMaxLimitAngle>=PI);
...
//			const PxQuat q = pose0.q * pose1.q.getConjugate();
			...
			{
//				PxD6Joint* j = PxD6JointCreate(physics, actor0, PxTransform(ToPxVec3(jc.mLocalPivot0)), actor1, PxTransform(ToPxVec3(jc.mLocalPivot1)));
				PxD6Joint* j = PxD6JointCreate(physics, actor0, PxTransform(ToPxVec3(jc.mLocalPivot0)), actor1, PxTransform(ToPxVec3(jc.mLocalPivot1), q));
				ASSERT(j);
...
//				PxFixedJoint* j = PxFixedJointCreate(physics, actor0, PxTransform(ToPxVec3(jc.mLocalPivot0), q), actor1, PxTransform(ToPxVec3(jc.mLocalPivot1)));
				PxFixedJoint* j = PxFixedJointCreate(physics, actor0, PxTransform(ToPxVec3(jc.mLocalPivot0)), actor1, PxTransform(ToPxVec3(jc.mLocalPivot1), q));
				ASSERT(j);
...
//	LocalPivot1.x += Range*0.5f;
	...
	//						const PxJointLinearLimitPair Limits(PxTolerancesScale(), jc.mMinLimit, jc.mMaxLimit, ContactDistance);


							PxJointLinearLimitPair Limits(jc.mMinLimit, jc.mMaxLimit, PxSpring(jc.mSpringStiffness, jc.mSpringDamping));
							if(jc.mSpringStiffness==0.0f && jc.mSpringDamping==0.0f)
							{
								//### these hard joints are better looking against limits but don't support "springs", at least in this form
								Limits = PxJointLinearLimitPair(PxTolerancesScale(), jc.mMinLimit, jc.mMaxLimit, ContactDistance);
							}
	...
	*/
			...
			{
				PxPrismaticJoint* j = PxPrismaticJointCreate(physics,	actor0, PxTransform(ToPxVec3(jc.mLocalPivot0.mPos), q0),
																		actor1, PxTransform(ToPxVec3(jc.mLocalPivot1.mPos), q1));
				ASSERT(j);
...
//#define TEMP_TEST
			...
			PxDistanceJoint* j = PxDistanceJointCreate(physics, actor0, PxTransform(ToPxVec3(jc.mLocalPivot0), q0), actor1, PxTransform(ToPxVec3(jc.mLocalPivot1), q1));
#else
...
void SharedPhysX::InitCommon()
{
	...
	{
		const PINT_MATERIAL_CREATE Desc(mParams.mDefaultFriction, mParams.mDefaultFriction, 0.0f);
...
//		mStaticActors.push_back(rigidStatic);
	...
	CreateShapes(desc.mShapes, actor, desc.mCollisionGroup, null);
...
bool SharedPhysX::GetHingeInfo(PintJointHandle handle, PintHingeInfo& info)
{
#ifdef IS_PHYSX_3_2
#else
	const PxJoint* J = reinterpret_cast<const PxJoint*>(handle);
	#ifdef IS_PHYSX_3_2
	if(J->getType()==PxJointType::eREVOLUTE)
	#else
	if(J->getConcreteType()==PxJointConcreteType::eREVOLUTE)
	#endif
	{
		const PxRevoluteJoint* Hinge = static_cast<const PxRevoluteJoint*>(J);
		info.mTwistAngle = Hinge->getAngle();
		return true;
	}
	#ifdef IS_PHYSX_3_2
	else if(J->getType()==PxJointType::eD6)
	#else
	else if(J->getConcreteType()==PxJointConcreteType::eD6)
	#endif
	{
		const PxD6Joint* D6 = static_cast<const PxD6Joint*>(J);
		info.mTwistAngle = D6->getTwist();
		return true;
	}
#endif
	return false;
...
bool SharedPhysX::GetD6Info(PintJointHandle handle, PintD6Info& info)
{
#ifdef IS_PHYSX_3_2
	return false;
#else
	...
	const PxD6Joint* D6 = static_cast<const PxD6Joint*>(J);
	info.mTwistAngle = D6->getTwist();
	info.mSwingYAngle = D6->getSwingYAngle();
	info.mSwingZAngle = D6->getSwingZAngle();
	return true;
#endif
}
...
{
	PxConvexMesh* ConvexMesh = ::CreatePhysXConvex(mPhysics, mCooking, desc.mNbVerts, desc.mVerts, PxConvexFlag::eCOMPUTE_CONVEX);
...
/*	const InternalSphereShape iss(Radius, &material, create->mRenderer, local_pose, collision_group);
...
#else
	{
		const udword Size = udword(mSphereShapes.size());
		for(udword i=0;i<Size;i++)
		{
			const InternalSphereShape& CurrentShape = mSphereShapes[i];
			if(		CurrentShape.mRadius==Radius
				&&	CurrentShape.Compare(&material, create->mRenderer, local_pose, collision_group))
			{
	...
	mSphereShapes.push_back(InternalSphereShape(Radius, NewShape, &material, create->mRenderer, local_pose, collision_group));
	return NewShape;
#endif
}
...
PxShape* SharedPhysX::CreateBoxShape(const PINT_SHAPE_CREATE* create, PxRigidActor* actor, const PxBoxGeometry& geometry, const PxMaterial& material, const PxTransform& local_pose, PxU16 collision_group)
{
	...
	{
		const udword Size = udword(mBoxShapes.size());
		for(udword i=0;i<Size;i++)
		{
	...
	mBoxShapes.push_back(InternalBoxShape(geometry.halfExtents, NewShape, &material, create->mRenderer, local_pose, collision_group));
	return NewShape;
}
...
PxShape* SharedPhysX::CreateCapsuleShape(const PINT_SHAPE_CREATE* create, PxRigidActor* actor, const PxCapsuleGeometry& geometry, const PxMaterial& material, const PxTransform& local_pose, PxU16 collision_group)
{
	...
	{
		const udword Size = udword(mCapsuleShapes.size());
		for(udword i=0;i<Size;i++)
		{
	...
	mCapsuleShapes.push_back(InternalCapsuleShape(geometry.radius, geometry.halfHeight, NewShape, &material, create->mRenderer, local_pose, collision_group));
	return NewShape;
}
...
//const PxCustomGeometryExt::CylinderData* CustomCylinder = cb->get<PxCustomGeometryExt::CylinderData>();
	...
	{
		const udword Size = udword(mCylinderShapes.size());
		for(udword i=0;i<Size;i++)
		{
			const InternalCapsuleShape& CurrentShape = mCylinderShapes[i];
			if(		CurrentShape.mRadius==CustomCylinder->radius
//			if(		CurrentShape.mRadius==CustomCylinder->getRadius()
				&&	CurrentShape.mHalfHeight==CustomCylinder->height
//				&&	CurrentShape.mHalfHeight==CustomCylinder->getHeight()
				&&	CurrentShape.Compare(&material, create->mRenderer, local_pose, collision_group))
			{
				actor->attachShape(*CurrentShape.mShape);
				return CurrentShape.mShape;
	...
	mCylinderShapes.push_back(InternalCapsuleShape(CustomCylinder->radius, CustomCylinder->height, NewShape, &material, create->mRenderer, local_pose, collision_group));
//mCylinderShapes.push_back(InternalCapsuleShape(CustomCylinder->getRadius(), CustomCylinder->getHeight(), NewShape, &material, create->mRenderer, local_pose, collision_group));
	return NewShape;
}
#endif
...
PxShape* SharedPhysX::CreateConvexShape(const PINT_SHAPE_CREATE* create, PxRigidActor* actor, const PxConvexMeshGeometry& geometry, const PxMaterial& material, const PxTransform& local_pose, PxU16 collision_group)
{
	...
	{
		const udword Size = udword(mConvexShapes.size());
		for(udword i=0;i<Size;i++)
		{
	...
	mConvexShapes.push_back(InternalConvexShape(geometry.convexMesh, NewShape, &material, create->mRenderer, local_pose, collision_group));
	return NewShape;
}
...
PxShape* SharedPhysX::CreateMeshShape(const PINT_SHAPE_CREATE* create, PxRigidActor* actor, const PxTriangleMeshGeometry& geometry, const PxMaterial& material, const PxTransform& local_pose, PxU16 collision_group)
{
	...
	{
		const udword Size = udword(mMeshShapes.size());
		for(udword i=0;i<Size;i++)
		{
			const InternalMeshShape& CurrentShape = mMeshShapes[i];
			if(		CurrentShape.mTriangleMesh==geometry.triangleMesh
				&&	CurrentShape.Compare(&material, create->mRenderer, local_pose, collision_group))
			{
				actor->attachShape(*CurrentShape.mShape);
				return CurrentShape.mShape;
	...
	mMeshShapes.push_back(InternalMeshShape(geometry.triangleMesh, NewShape, &material, create->mRenderer, local_pose, collision_group));
	return NewShape;
}
...
PxShape* SharedPhysX::CreateHeightfieldShape(const PINT_SHAPE_CREATE* create, PxRigidActor* actor, const PxHeightFieldGeometry& geometry, const PxMaterial& material, const PxTransform& local_pose, PxU16 collision_group)
{
	...
	{
		const udword Size = udword(mHeightfieldShapes.size());
		for(udword i=0;i<Size;i++)
		{
			// #### maybe missing scales here
			const InternalHeightfieldShape& CurrentShape = mHeightfieldShapes[i];
			if(		CurrentShape.mHeightfield==geometry.heightField
				&&	CurrentShape.Compare(&material, create->mRenderer, local_pose, collision_group))
			{
				actor->attachShape(*CurrentShape.mShape);
				return CurrentShape.mShape;
	...
	mHeightfieldShapes.push_back(InternalHeightfieldShape(geometry.heightField, NewShape, &material, create->mRenderer, local_pose, collision_group));
	return NewShape;
}
#endif

void SharedPhysX::CreateShapes(const PINT_SHAPE_CREATE* create, PxRigidActor* actor, PintCollisionGroup group, const char* forced_name)
{
	ASSERT(actor);
	ASSERT(mDefaultMaterial);

	const bool IsDynamic = actor->getConcreteType()==PxConcreteType::eRIGID_DYNAMIC;
	...
	while(CurrentShape)
	{
		...
		if(CurrentShape->mType==PINT_SHAPE_SPHERE)
		{
			...
			shape = CreateSphereShape(CurrentShape, actor, PxSphereGeometry(SphereCreate->mRadius), *ShapeMaterial, LocalPose, group);
			//shape = CreateCapsuleShape(CurrentShape, actor, PxCapsuleGeometry(SphereCreate->mRadius, 0.0f), *ShapeMaterial, LocalPose, group);
		}
		else if(CurrentShape->mType==PINT_SHAPE_BOX)
		{
			const PINT_BOX_CREATE* BoxCreate = static_cast<const PINT_BOX_CREATE*>(CurrentShape);
			shape = CreateBoxShape(CurrentShape, actor, PxBoxGeometry(ToPxVec3(BoxCreate->mExtents)), *ShapeMaterial, LocalPose, group);
		}
...
//			LocalPose.q = q * LocalPose.q;
			...
			shape = CreateCapsuleShape(CurrentShape, actor, PxCapsuleGeometry(CapsuleCreate->mRadius, CapsuleCreate->mHalfHeight), *ShapeMaterial, LocalPose, group);
		}
#if PHYSX_SUPPORT_CUSTOM_GEOMETRY
...
const PxCustomGeometry CustomGeom(*cb);
			...
			shape = CreateCylinderShape(CurrentShape, actor, CustomGeom, *ShapeMaterial, LocalPose, group);
		}
#endif
...
//			PxConvexMesh* ConvexMesh = CreateConvexMesh(ConvexCreate->mVerts, ConvexCreate->mNbVerts, PxConvexFlag::eCOMPUTE_CONVEX, CurrentShape->mRenderer);
			PxConvexMesh* ConvexMesh = CreateConvexMesh(ConvexCreate->mVerts, ConvexCreate->mNbVerts, PxConvexFlag::eCOMPUTE_CONVEX, CurrentShape->mRenderer, mParams.mShareMeshData);
			ASSERT(ConvexMesh);
...
#endif
			shape = CreateConvexShape(CurrentShape, actor, ConvexGeom, *ShapeMaterial, LocalPose, group);
		}
		else if(CurrentShape->mType==PINT_SHAPE_MESH)
		{
			...
			ASSERT(mCooking);
			PxTriangleMesh* TriangleMesh = CreateTriangleMesh(MeshCreate->GetSurface(), CurrentShape->mRenderer, MeshCreate->mDeformable, mParams.mShareMeshData, IsDynamic);
			ASSERT(TriangleMesh);

			shape = CreateMeshShape(CurrentShape, actor, PxTriangleMeshGeometry(TriangleMesh), *ShapeMaterial, LocalPose, group);

//						PxTriangleMeshGeometry meshGeom(TriangleMesh);
//						meshGeom.scale.scale = PxVec3(-1.0f);
//						shape = CreateMeshShape(CurrentShape, actor, meshGeom, *ShapeMaterial, LocalPose, group);
		}
...
#if PHYSX_SUPPORT_HEIGHTFIELDS
		else if(CurrentShape->mType==PINT_SHAPE_HEIGHTFIELD)
		{
			const PINT_HEIGHTFIELD_CREATE* HeightfieldCreate = static_cast<const PINT_HEIGHTFIELD_CREATE*>(CurrentShape);
			...
			shape = CreateHeightfieldShape(CurrentShape, actor, hfGeom, *ShapeMaterial, LocalPose, group);
		}
#endif
		else ASSERT(0);
...
static __forceinline void RenderShape(PintRender& renderer, PxShape* shape, const PxTransform& shapePose)
{
...
//	PHYSX_GUI_NUM_16K_CONTACT_DATA_BLOCKS,
	...
	PHYSX_GUI_FRICTION_OFFSET_THRESHOLD,
#if PHYSX_SUPPORT_TORSION_FRICTION
	PHYSX_GUI_TORSIONAL_PATCH_RADIUS,
	PHYSX_GUI_MIN_TORSIONAL_PATCH_RADIUS,
#endif
	PHYSX_GUI_CONTACT_OFFSET,
	PHYSX_GUI_REST_OFFSET,
#if PHYSX_SUPPORT_CONTACT_NOTIFICATIONS
	PHYSX_GUI_CONTACT_NOTIF_THRESHOLD,
#endif
...
#if PHYSX_SUPPORT_STABILIZATION_FLAG
	PHYSX_GUI_STABILIZATION_THRESHOLD,
#endif
#if PHYSX_SUPPORT_PX_BROADPHASE_TYPE
...
//	PHYSX_GUI_ENABLE_JOINT_CONTACT_DISTANCE,
...
#endif
#if PHYSX_SUPPORT_RCA_CFM_SCALE
	PHYSX_GUI_RCA_CFM_SCALE,
#endif
#if PHYSX_SUPPORT_RCA_DOF_SCALE
	PHYSX_GUI_RCA_DOF_SCALE,
#endif
#if PHYSX_SUPPORT_RCA_ARMATURE
	PHYSX_GUI_RCA_ARMATURE,
#endif
	// Cooking
#if PHYSX_SUPPORT_PX_MESH_COOKING_HINT
	...
	PHYSX_GUI_STEER_FILTER,
#endif
#if PHYSX_SUPPORT_VEHICLE_SWEEP_INFLATION
	PHYSX_GUI_VEHICLE_SWEEP_INFLATION,
#endif
	//
...
#define MAX_NB_DEBUG_VIZ_PARAMS	32
	...
	struct PhysXUI : public Allocateable
	{
		...
		IceComboBox*	mComboBox_LimitsContactDistance;
#if PHYSX_SUPPORT_SCRATCH_BUFFER
...
#endif
#if PHYSX_SUPPORT_PX_MESH_MIDPHASE2
		IceEditBox*		mEditBox_NbTrisPerLeaf;
#endif
#if PHYSX_SUPPORT_PX_MESH_BUILD_STRATEGY
		IceComboBox*	mComboBox_MeshBuildStrategy;
#endif
#if PHYSX_SUPPORT_PX_MESH_COOKING_HINT
...
#ifdef PHYSX_SUPPORT_LINEAR_COEFF
		IceEditBox*		mEditBox_LinearCoeff;
#endif
#if PHYSX_SUPPORT_ARTICULATIONS
...
#endif
#if PHYSX_SUPPORT_RCA_CFM_SCALE
		IceEditBox*		mEditBox_RCACfmScale;
#endif
#if PHYSX_SUPPORT_RCA_DOF_SCALE
		IceEditBox*		mEditBox_RCADofScale;
#endif
#if PHYSX_SUPPORT_RCA_ARMATURE
		IceEditBox*		mEditBox_RCAArmature;
#endif
		...
		IceEditBox*		mEditBox_FrictionOffsetThreshold;
#if PHYSX_SUPPORT_TORSION_FRICTION
		IceEditBox*		mEditBox_TorsionalPatchRadius;
		IceEditBox*		mEditBox_MinTorsionalPatchRadius;
#endif
		IceEditBox*		mEditBox_ContactOffset;
		IceEditBox*		mEditBox_RestOffset;
#if PHYSX_SUPPORT_CONTACT_NOTIFICATIONS
		IceEditBox*		mEditBox_ContactNotifThreshold;
#endif
#if PHYSX_SUPPORT_SUBSTEPS
...
#if PHYSX_SUPPORT_STABILIZATION_FLAG
		IceEditBox*		mEditBox_StabilizationThreshold;
#endif
#if PHYSX_SUPPORT_USER_DEFINED_GAUSSMAP_LIMIT
		IceEditBox*		mEditBox_GaussMapLimit;
#endif
#if PHYSX_SUPPORT_QUANTIZED_TREE_OPTION
		IceCheckBox*	mCheckBox_QuantizedTrees;
#endif
#if PHYSX_SUPPORT_IMPROVED_PATCH_FRICTION
		IceCheckBox*	mCheckBox_ImprovedPatchFriction;
#endif
#if PHYSX_SUPPORT_FRICTION_EVERY_ITERATION
		IceCheckBox*	mCheckBox_FrictionEveryIteration;
#endif
#if PHYSX_SUPPORT_STABILIZATION_FLAG
		IceCheckBox*	mCheckBox_Stabilization;
#endif
#if PHYSX_SUPPORT_TGS
		IceCheckBox*	mCheckBox_TGS;
#endif
#if PHYSX_SUPPORT_GYROSCOPIC_FORCES
		IceCheckBox*	mCheckBox_Gyro;
#endif
		IceCheckBox*	mCheckBox_CCD;
#if PHYSX_SUPPORT_ANGULAR_CCD
		IceCheckBox*	mCheckBox_AngularCCD;
#endif
#if PHYSX_SUPPORT_RAYCAST_CCD
		IceCheckBox*	mCheckBox_RaycastCCDStatic;
		IceCheckBox*	mCheckBox_RaycastCCDDynamic;
#endif
...
#if PHYSX_SUPPORT_VEHICLE_SUSPENSION_SWEEPS
		IceCheckBox*	mCheckBox_UseSuspensionSweeps;
		IceCheckBox*	mCheckBox_ForceSphereWheels;
		IceCheckBox*	mCheckBox_UseBlockingSweeps;
		IceCheckBox*	mCheckBox_DrawSweptWheels;
#endif
		IceCheckBox*	mCheckBox_DrawSuspensionCasts;
		IceCheckBox*	mCheckBox_DrawPhysXTelemetry;
		IceCheckBox*	mCheckBox_UseFix;
#if PHYSX_SUPPORT_STEER_FILTER
		IceEditBox*		mEditBox_SteerFilter;
#endif
#if PHYSX_SUPPORT_VEHICLE_SWEEP_INFLATION
		IceEditBox*		mEditBox_SweepInflation;
#endif
		IceEditBox*		mEditBox_DebugVizScale;
...
PhysXUI::PhysXUI(UICallback& callback, udword nb_debug_viz_params, bool* debug_viz_params, const char** debug_viz_names) :
	...
	mComboBox_LimitsContactDistance		(null),
#if PHYSX_SUPPORT_SCRATCH_BUFFER
...
#endif
#if PHYSX_SUPPORT_PX_MESH_MIDPHASE2
	mEditBox_NbTrisPerLeaf				(null),
#endif
#if PHYSX_SUPPORT_PX_MESH_BUILD_STRATEGY
	mComboBox_MeshBuildStrategy			(null),
#endif
#if PHYSX_SUPPORT_PX_MESH_COOKING_HINT
...
#endif
#if PHYSX_SUPPORT_RCA_CFM_SCALE
	mEditBox_RCACfmScale				(null),
#endif
#if PHYSX_SUPPORT_RCA_DOF_SCALE
	mEditBox_RCADofScale				(null),
#endif
#if PHYSX_SUPPORT_RCA_ARMATURE
	mEditBox_RCAArmature				(null),
#endif
	...
	mEditBox_FrictionOffsetThreshold	(null),
#if PHYSX_SUPPORT_TORSION_FRICTION
	mEditBox_TorsionalPatchRadius		(null),
	mEditBox_MinTorsionalPatchRadius	(null),
#endif
	mEditBox_ContactOffset				(null),
	mEditBox_RestOffset					(null),
#if PHYSX_SUPPORT_CONTACT_NOTIFICATIONS
	mEditBox_ContactNotifThreshold		(null),
#endif
#if PHYSX_SUPPORT_SUBSTEPS
...
#if PHYSX_SUPPORT_STABILIZATION_FLAG
	mEditBox_StabilizationThreshold		(null),
#endif
#if PHYSX_SUPPORT_USER_DEFINED_GAUSSMAP_LIMIT
	mEditBox_GaussMapLimit				(null),
#endif
#if PHYSX_SUPPORT_QUANTIZED_TREE_OPTION
	mCheckBox_QuantizedTrees			(null),
#endif
#if PHYSX_SUPPORT_IMPROVED_PATCH_FRICTION
	mCheckBox_ImprovedPatchFriction		(null),
#endif
#if PHYSX_SUPPORT_FRICTION_EVERY_ITERATION
	mCheckBox_FrictionEveryIteration	(null),
#endif
#if PHYSX_SUPPORT_STABILIZATION_FLAG
	mCheckBox_Stabilization				(null),
#endif
#if PHYSX_SUPPORT_TGS
	mCheckBox_TGS						(null),
#endif
	...
	mCheckBox_CCD						(null),
#if PHYSX_SUPPORT_ANGULAR_CCD
	mCheckBox_AngularCCD				(null),
#endif
#if PHYSX_SUPPORT_RAYCAST_CCD
	mCheckBox_RaycastCCDStatic			(null),
	mCheckBox_RaycastCCDDynamic			(null),
#endif
...
#if PHYSX_SUPPORT_VEHICLE_SUSPENSION_SWEEPS
	mCheckBox_UseSuspensionSweeps		(null),
	mCheckBox_ForceSphereWheels			(null),
	mCheckBox_UseBlockingSweeps			(null),
	mCheckBox_DrawSweptWheels			(null),
#endif
	mCheckBox_DrawSuspensionCasts		(null),
	mCheckBox_DrawPhysXTelemetry		(null),
	mCheckBox_UseFix					(null),
#if PHYSX_SUPPORT_STEER_FILTER
	mEditBox_SteerFilter				(null),
#endif
#if PHYSX_SUPPORT_VEHICLE_SWEEP_INFLATION
	mEditBox_SweepInflation				(null),
#endif
	mEditBox_DebugVizScale				(null)
{
...
//		gDebugVizParams[i] = gPhysXUI->mCheckBox_DebugVis[i]->IsChecked();
	...
	{
		if(1)//###DEMO	// Hardcoded for demo script
		{
...
#if PHYSX_SUPPORT_CONTACT_NOTIFICATIONS
			helper.CreateLabel(TabWindow, x2, y+LabelOffsetY, LabelWidth, 20, "Contact notif threshold:", gPhysXUI->mPhysXGUI);
			gPhysXUI->mEditBox_ContactNotifThreshold = helper.CreateEditBox(TabWindow, PHYSX_GUI_CONTACT_NOTIF_THRESHOLD, x2+EditBoxX, y, EditBoxWidth, 20, helper.Convert(gParams.mContactNotifThreshold), gPhysXUI->mPhysXGUI, EDITBOX_FLOAT, null);
			y += YStep;
#endif
...
//											gPhysXUI->mCheckBox_TGS->SetChecked(gParams.mTGS);
		...
		// TAB_DYNAMICS
		{
...

			{
				...
				helper.CreateLabel(TabWindow, xf2, y+LabelOffsetY, LabelWidth2, 20, "Default friction coeff:", gPhysXUI->mPhysXGUI);
				gPhysXUI->mEditBox_DefaultFriction = helper.CreateEditBox(TabWindow, PHYSX_GUI_DEFAULT_FRICTION, xf2+EditBoxX2, y, EditBoxWidth, 20, helper.Convert(gParams.mDefaultFriction), gPhysXUI->mPhysXGUI, EDITBOX_FLOAT_POSITIVE, null);
				y += YStep;

				helper.CreateLabel(TabWindow, xf2, y+LabelOffsetY, LabelWidth2, 20, "Friction offset threshold:", gPhysXUI->mPhysXGUI);
				gPhysXUI->mEditBox_FrictionOffsetThreshold = helper.CreateEditBox(TabWindow, PHYSX_GUI_FRICTION_OFFSET_THRESHOLD, xf2+EditBoxX2, y, EditBoxWidth, 20, helper.Convert(gParams.mFrictionOffsetThreshold), gPhysXUI->mPhysXGUI, EDITBOX_FLOAT_POSITIVE, null);
				y += YStep;

#if PHYSX_SUPPORT_TORSION_FRICTION
				helper.CreateLabel(TabWindow, xf2, y+LabelOffsetY, LabelWidth2, 20, "Torsional patch radius:", gPhysXUI->mPhysXGUI);
				gPhysXUI->mEditBox_TorsionalPatchRadius = helper.CreateEditBox(TabWindow, PHYSX_GUI_TORSIONAL_PATCH_RADIUS, xf2+EditBoxX2, y, EditBoxWidth, 20, helper.Convert(gParams.mTorsionalPatchRadius), gPhysXUI->mPhysXGUI, EDITBOX_FLOAT_POSITIVE, null);
				y += YStep;

				helper.CreateLabel(TabWindow, xf2, y+LabelOffsetY, LabelWidth2, 20, "Min torsional patch radius:", gPhysXUI->mPhysXGUI);
				gPhysXUI->mEditBox_MinTorsionalPatchRadius = helper.CreateEditBox(TabWindow, PHYSX_GUI_MIN_TORSIONAL_PATCH_RADIUS, xf2+EditBoxX2, y, EditBoxWidth, 20, helper.Convert(gParams.mMinTorsionalPatchRadius), gPhysXUI->mPhysXGUI, EDITBOX_FLOAT_POSITIVE, null);
				y += YStep;
#endif
			...
			gPhysXUI->mEditBox_SleepThreshold = helper.CreateEditBox(TabWindow, PHYSX_GUI_SLEEP_THRESHOLD, 4+EditBoxX, y, EditBoxWidth, 20, helper.Convert(gParams.mSleepThreshold), gPhysXUI->mPhysXGUI, EDITBOX_FLOAT_POSITIVE, null);
			y += YStep;
#if PHYSX_SUPPORT_STABILIZATION_FLAG
			helper.CreateLabel(TabWindow, 4, y+LabelOffsetY, LabelWidth, 20, "Stabilization threshold:", gPhysXUI->mPhysXGUI);
			gPhysXUI->mEditBox_StabilizationThreshold = helper.CreateEditBox(TabWindow, PHYSX_GUI_STABILIZATION_THRESHOLD, 4+EditBoxX, y, EditBoxWidth, 20, helper.Convert(gParams.mStabilizationThreshold), gPhysXUI->mPhysXGUI, EDITBOX_FLOAT_POSITIVE, null);
			y += YStep;
#endif
		}
...
#ifdef PHYSX_SUPPORT_LINEAR_COEFF
			helper.CreateLabel(TabWindow, xj, y+LabelOffsetY, LabelWidthJ, 20, "Linear coefficient:", gPhysXUI->mPhysXGUI);
			gPhysXUI->mEditBox_LinearCoeff = helper.CreateEditBox(TabWindow, PHYSX_GUI_LINEAR_COEFF, xj+LabelWidthJ, y, EditBoxWidth, 20, _F("%f", gParams.mLinearCoeff), gPhysXUI->mPhysXGUI, EDITBOX_FLOAT_POSITIVE, null);
			y += YStep;
#endif
...
#if PHYSX_SUPPORT_RCA_CFM_SCALE
				helper.CreateLabel(TabWindow, xa, y+LabelOffsetY, LabelWidthJ, 20, "RCA CFM scale:", gPhysXUI->mPhysXGUI);
				gPhysXUI->mEditBox_RCACfmScale = helper.CreateEditBox(TabWindow, PHYSX_GUI_RCA_CFM_SCALE, xj+LabelWidthJ, y, EditBoxWidth, 20, _F("%f", gParams.mRCACfmScale), gPhysXUI->mPhysXGUI, EDITBOX_FLOAT_POSITIVE, null);
				y += YStep;
#endif
#if PHYSX_SUPPORT_RCA_DOF_SCALE
				helper.CreateLabel(TabWindow, xa, y+LabelOffsetY, LabelWidthJ, 20, "RCA DOF scale:", gPhysXUI->mPhysXGUI);
				gPhysXUI->mEditBox_RCADofScale = helper.CreateEditBox(TabWindow, PHYSX_GUI_RCA_DOF_SCALE, xj+LabelWidthJ, y, EditBoxWidth, 20, _F("%f", gParams.mRCADofScale), gPhysXUI->mPhysXGUI, EDITBOX_FLOAT_POSITIVE, null);
				y += YStep;
#endif
#if PHYSX_SUPPORT_RCA_ARMATURE
				helper.CreateLabel(TabWindow, xa, y+LabelOffsetY, LabelWidthJ, 20, "RCA armature:", gPhysXUI->mPhysXGUI);
				gPhysXUI->mEditBox_RCAArmature = helper.CreateEditBox(TabWindow, PHYSX_GUI_RCA_ARMATURE, xj+LabelWidthJ, y, EditBoxWidth, 20, _F("%f", gParams.mRCAArmature), gPhysXUI->mPhysXGUI, EDITBOX_FLOAT_POSITIVE, null);
				y += YStep;
#endif
				{
					// Cannot use helper function here because "mType" is missing from wrapper.
...
#endif
#if PHYSX_SUPPORT_PX_MESH_MIDPHASE2
			helper.CreateLabel(TabWindow, 4, y+LabelOffsetY, 90, 20, "Nb tris per leaf:", gPhysXUI->mPhysXGUI);
			gPhysXUI->mEditBox_NbTrisPerLeaf = helper.CreateEditBox(TabWindow, PHYSX_GUI_NB_TRIS_PER_LEAF, 4+EditBoxX, y, EditBoxWidth, 20, _F("%d", gParams.mNbTrisPerLeaf), gPhysXUI->mPhysXGUI, EDITBOX_INTEGER_POSITIVE, null);
			y += YStep;
#endif
			y += YStepCB;
#if PHYSX_SUPPORT_USER_DEFINED_GAUSSMAP_LIMIT
...
#if PHYSX_SUPPORT_STEER_FILTER
			y += YStep;
			helper.CreateLabel(TabWindow, 4, y+LabelOffsetY, 90, 20, "Steer filter:", gPhysXUI->mPhysXGUI);
			gPhysXUI->mEditBox_SteerFilter = helper.CreateEditBox(TabWindow, PHYSX_GUI_STEER_FILTER, 4+EditBoxX, y, EditBoxWidth, 20, _F("%.3f", gParams.mSteerFilter), gPhysXUI->mPhysXGUI, EDITBOX_FLOAT_POSITIVE, null);
			y += YStep;
#endif
#if PHYSX_SUPPORT_VEHICLE_SWEEP_INFLATION
			y += YStep;
			helper.CreateLabel(TabWindow, 4, y+LabelOffsetY, 90, 20, "Sweep inflation:", gPhysXUI->mPhysXGUI);
			gPhysXUI->mEditBox_SweepInflation = helper.CreateEditBox(TabWindow, PHYSX_GUI_VEHICLE_SWEEP_INFLATION, 4+EditBoxX, y, EditBoxWidth, 20, _F("%.3f", gParams.mSweepInflation), gPhysXUI->mPhysXGUI, EDITBOX_FLOAT_POSITIVE, null);
			y += YStep;
#endif
