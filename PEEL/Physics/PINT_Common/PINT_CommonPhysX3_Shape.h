...
/*			if(mMaterial!=&material)
	...
	class PhysX_ShapeAPI : public Pint_Shape
	{
		...
		virtual	bool				FindTouchedTriangles(Container& indices, PintSQThreadContext context, PintShapeHandle handle, const PR& pose, const PintSphereOverlapData& sphere)	const;
		virtual	bool				Refit(PintShapeHandle shape, PintActorHandle actor);

		virtual	PintShapeRenderer*	GetShapeRenderer(PintShapeHandle handle)	const;
	};
