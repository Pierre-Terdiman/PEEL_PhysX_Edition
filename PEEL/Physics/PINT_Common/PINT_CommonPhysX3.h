...
//		bool							mEnableJointContactDistance;
	...
	class SharedPhysX : public Pint, public MeshManager
#ifndef IS_PHYSX_3_2
...
//		virtual	void						Close();
		...
		virtual	bool						ReleaseJoint(PintJointHandle handle);

#ifndef IS_PHYSX_3_2
		virtual	bool						SetSQFlag(PintActorHandle actor, bool flag);
		virtual	bool						SetSQFlag(PintActorHandle actor, PintShapeHandle shape, bool flag);
		virtual	bool						ResetSQFilters();
#endif
...
//		virtual	void						ApplyActionAtPoint(PintActorHandle handle, PintActionType action_type, const Point& action, const Point& pos);
		...
		virtual	void						AddLocalTorque(PintActorHandle handle, const Point& local_torque);
		...
		virtual	bool						SetKinematicPose(PintActorHandle handle, const PR& pr);
		virtual	bool						IsKinematic(PintActorHandle handle);
		virtual	bool						EnableKinematic(PintActorHandle handle, bool flag);

		virtual	PintConvexHandle			CreateConvexObject(const PINT_CONVEX_DATA_CREATE& desc, PintConvexIndex* index);
		virtual	bool						DeleteConvexObject(PintConvexHandle handle, const PintConvexIndex* index);
		virtual	PintMeshHandle				CreateMeshObject(const PINT_MESH_DATA_CREATE& desc, PintMeshIndex* index);
		virtual	bool						DeleteMeshObject(PintMeshHandle handle, const PintMeshIndex* index);
#if PHYSX_SUPPORT_HEIGHTFIELDS
		virtual	PintHeightfieldHandle		CreateHeightfieldObject(const PINT_HEIGHTFIELD_DATA_CREATE& desc, PintHeightfieldData& data, PintHeightfieldIndex* index);
		virtual	bool						DeleteHeightfieldObject(PintHeightfieldHandle handle, const PintHeightfieldIndex* index);
#endif
		...
		virtual	bool						AddAggregateToScene(PintAggregateHandle aggregate);
...
//		virtual	bool						Picking						(PintSQThreadContext context, PintRaycastHit* dest, const PintRaycastData& raycast, PintQueryFilterCallback* cb);
		...
		virtual	bool						GetHingeInfo(PintJointHandle handle, PintHingeInfo& info);
		virtual	bool						GetD6Info(PintJointHandle handle, PintD6Info& info);
		virtual	bool						SetDriveEnabled(PintJointHandle handle, bool flag);
		virtual	bool						SetDrivePosition(PintJointHandle handle, const PR& pose);
...
//		virtual	void						SetGearJointError(PintJointHandle handle, float error);
		...
		virtual	Pint_Scene*					GetSceneAPI()		{ return &mSceneAPI;	}
		virtual	Pint_Actor*					GetActorAPI()		{ return &mActorAPI;	}
		virtual	Pint_Shape*					GetShapeAPI()		{ return &mShapeAPI;	}
		virtual	Pint_Joint*					GetJointAPI()		{ return &mJointAPI;	}
...
#endif
#ifdef PHYSX_SUPPORT_CHARACTERS2
		virtual	Pint_Character*				GetCharacterAPI()	{ return &mCCT;	}
#endif
...
//				PxTriangleMesh*				CreateTriangleMesh(const SurfaceInterface& surface, PintShapeRenderer* renderer, bool deformable);
		...
		protected:
				...
				std::vector<InternalCapsuleShape>	mCylinderShapes;
#endif
				std::vector<InternalConvexShape>	mConvexShapes;
				std::vector<InternalMeshShape>		mMeshShapes;
#if PHYSX_SUPPORT_HEIGHTFIELDS
				std::vector<InternalHeightfieldShape>	mHeightfieldShapes;
#endif

#ifdef IS_PHYSX_3_2
		// Virtual on purpose, overriden in PhysX3.2 Pint plugin
		virtual	void								CreateShapes			(const PINT_SHAPE_CREATE* create, PxRigidActor* actor, PintCollisionGroup group, const char* forced_name){}
#else
		private:
...
#endif
		protected:
				void								CreateShapes		(const PINT_SHAPE_CREATE* create, PxRigidActor* actor, PintCollisionGroup group, const char* forced_name);
#endif
