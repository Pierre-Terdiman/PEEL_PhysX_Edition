...
//	DeleteOwnedObjects<PINT_AGGREGATE_CREATE>(mAggregates);
	...
	DeleteOwnedObjects<PINT_OBJECT_CREATE>(mActors);
	DeleteOwnedObjects<PINT_SHAPE_CREATE>(mShapes);
...
void ZCB2Factory::SetupShape(PINT_SHAPE_CREATE* create, const ShapeChunk& chunk, const char* name)
{
	...
	ASSERT(mShapes.GetNbEntries()==chunk.GetID());
	mShapes.AddPtr(create);
}

static void SetupJoint(PINT_JOINT_CREATE* create, const JointChunk& chunk, PtrContainer& joints, /*Container& joints_actor_indices, */const PtrContainer& actors, const char* name)
{
	create->mName		= name;
	...
	const bool ValidActor0 = chunk.mObject0<actors.GetNbEntries() || chunk.mObject0==INVALID_ID;
	const bool ValidActor1 = chunk.mObject1<actors.GetNbEntries() || chunk.mObject1==INVALID_ID;
	ASSERT(ValidActor0);
	ASSERT(ValidActor1);
...
//			PINT_MESH_DATA_CREATE* Create = ICE_NEW(PINT_MESH_DATA_CREATE);
		...
		case ActorType:
		{
			...
			for(udword i=0;i<NbShapes;i++)
			{
				...
				ASSERT(ShapeID<mShapes.GetNbEntries());
				PINT_SHAPE_CREATE* CurrentShape = (PINT_SHAPE_CREATE*)mShapes.GetEntry(ShapeID);
				if(!Dst)
					Create->mShapes = CurrentShape;
				else
				{
					ASSERT(!Dst->mNext || Dst->mNext==CurrentShape);
					Dst->mNext = CurrentShape;
				}
				Dst = CurrentShape;
//				ASSERT(!CurrentShape->mNext);
			}

			ASSERT(mActors.GetNbEntries()==Chunk.GetID());
			mActors.AddPtr(Create);
			const bool Hidden = Chunk.IsSetHidden();
			mActorsFlags.Add(udword(Hidden));
			if(mCustomize)
		...
		case FixedJointType:
		{
			...
			SetupJoint(Create, Chunk, mJoints, /*mJointsActorIndices, */mActors, name);
			if(mCustomize)
		...
		case DistanceJointType:
		{
			...
			Create->mLocalPivot0	= Chunk.mLocalPivot0;
			Create->mLocalPivot1	= Chunk.mLocalPivot1;
			Create->mMinDistance	= Chunk.mMinDistance;
			Create->mMaxDistance	= Chunk.mMaxDistance;
			SetupJoint(Create, Chunk, mJoints, /*mJointsActorIndices, */mActors, name);
			if(mCustomize)
				mCustomize->CustomizeJoint(*Create);
		}
...
//			{
				...
				Create->mMinLimit			= Chunk.mMinLimit;
				Create->mMaxLimit			= Chunk.mMaxLimit;
				Create->mSpringStiffness	= Chunk.mSpringStiffness;
				Create->mSpringDamping		= Chunk.mSpringDamping;
				SetupJoint(Create, Chunk, mJoints, /*mJointsActorIndices, */mActors, name);
//			}
//			else
//			{
...
//			counter++;
		...
		case HingeJointType:
		{
			...
			Create->mMinLimitAngle	= Chunk.mMinLimitAngle;
			Create->mMaxLimitAngle	= Chunk.mMaxLimitAngle;
...
#define SPHERICAL_JOINT_VERSION	1
...
DistanceJointChunk::DistanceJointChunk(const PINT_DISTANCE_JOINT_CREATE& create) : JointChunk(create)
{
	...
	mLocalPivot0				= create.mLocalPivot0;
	mLocalPivot1				= create.mLocalPivot1;
	mMinDistance				= create.mMinDistance;
	mMaxDistance				= create.mMaxDistance;
}
...
DistanceJointChunk::DistanceJointChunk(const DistanceJointData& data) : JointChunk(data)
{
	...
	mLocalPivot0				= data.mLocalPivot0;
	mLocalPivot1				= data.mLocalPivot1;
	mMinDistance				= data.mMinDistance;
	mMaxDistance				= data.mMaxDistance;
}
...
bool DistanceJointChunk::Export(CustomArray& array)
{
	...
	::Export(mLocalPivot0, array);
	::Export(mLocalPivot1, array);
	array.Store(mMinDistance).Store(mMaxDistance);
	return true;
}
...
bool DistanceJointChunk::Import(const VirtualFile& file)
{
	...
	::Import(mLocalPivot0, file);
	::Import(mLocalPivot1, file);
	mMinDistance = file.ReadFloat();
	mMaxDistance = file.ReadFloat();
	...
	if(gUseRenderScale && gZCB2_RenderScale!=1.0f)
	{
		...
		if(mMinDistance>0.0f)
			mMinDistance *= gZCB2_RenderScale;
		if(mMaxDistance>0.0f)
			mMaxDistance *= gZCB2_RenderScale;
	}
	return true;
}
...
PrismaticJointChunk::PrismaticJointChunk(const PINT_PRISMATIC_JOINT_CREATE& create) : JointChunk(create)
{
	...
	mMinLimit						= create.mMinLimit;
	mMaxLimit						= create.mMaxLimit;
	mSpringStiffness				= create.mSpringStiffness;
	mSpringDamping					= create.mSpringDamping;
}
...
PrismaticJointChunk::PrismaticJointChunk(const PrismaticJointData& data) : JointChunk(data)
{
	...
	mMinLimit						= data.mMinLimit;
	mMaxLimit						= data.mMaxLimit;
	mSpringStiffness				= data.mSpringStiffness;
	mSpringDamping					= data.mSpringDamping;
}
...
bool PrismaticJointChunk::Export(CustomArray& array)
{
	...
	array.Store(mMinLimit).Store(mMaxLimit).Store(mSpringStiffness).Store(mSpringDamping);
	return true;
}
...
bool PrismaticJointChunk::Import(const VirtualFile& file)
{
	...
	mMinLimit = file.ReadFloat();
	mMaxLimit = file.ReadFloat();
	mSpringStiffness = file.ReadFloat();
	mSpringDamping = file.ReadFloat();
...
//	mSpringDamping	= 0.0f;
	...
	if(gUseRenderScale && gZCB2_RenderScale!=1.0f)
	{
		...
		if(mMinLimit<=mMaxLimit)
		{
			mMinLimit *= gZCB2_RenderScale;
			mMaxLimit *= gZCB2_RenderScale;
		}
	}
...
HingeJointChunk::HingeJointChunk(const PINT_HINGE_JOINT_CREATE& create) : JointChunk(create)
{
	...
	mLocalAxis0					= create.mLocalAxis0;
	mLocalAxis1					= create.mLocalAxis1;
	mMinLimitAngle				= create.mMinLimitAngle;
	mMaxLimitAngle				= create.mMaxLimitAngle;
...
HingeJointChunk::HingeJointChunk(const HingeJointData& data) : JointChunk(data)
{
	...
	mLocalAxis0					= data.mLocalAxis0;
	mLocalAxis1					= data.mLocalAxis1;
	mMinLimitAngle				= data.mMinLimitAngle;
	mMaxLimitAngle				= data.mMaxLimitAngle;
...
bool HingeJointChunk::Export(CustomArray& array)
{
	...
	array.Store(mMinLimitAngle).Store(mMaxLimitAngle).Store(mDriveVelocity).Store(mUseMotor?udword(1):udword(0));
	return true;
}
...
bool HingeJointChunk::Import(const VirtualFile& file)
{
	...
	mMinLimitAngle = file.ReadFloat();
	mMaxLimitAngle = file.ReadFloat();
