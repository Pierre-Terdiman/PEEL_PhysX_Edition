...
static const char* gDesc_D6Joint_PrismaticDrive = "(Configurable test) - D6 joint - prismatic drive.";
...
//			IceComboBox*	mComboBox_Preset;
			...
			IceEditBox*		mEditBox_ObstacleMass;
			IceCheckBox*	mCheckBox_UsePositionDrive;
...
/*								mComboBox_Preset	(null),*/
								...
								mEditBox_ObstacleMass		(null),
								mCheckBox_UsePositionDrive	(null),
...
static const char* gDesc_D6Joint_PrismaticPosDrive = "(Configurable test) - D6 joint - prismatic position drive.";
...
//				Desc.mMaxSwingZ				= 120.0f*DEGTORAD;
	...
	virtual	float		DrawDebugText(Pint& pint, GLFontRenderer& renderer, float y, float text_scale)
	{
		...
		if(!pint.GetD6Info(Handle, Info))
			return y;
		renderer.print(0.0f, y, text_scale, _F("Swing Y angle: %.5f\n", Info.mSwingYAngle * RADTODEG));	y -= text_scale;
		renderer.print(0.0f, y, text_scale, _F("Swing Z angle: %.5f\n", Info.mSwingZAngle * RADTODEG));	y -= text_scale;
		return y;
	}
...
//		const Point LinVel(0.0f, 1.0f, 0.0f);
	...
	virtual	float		DrawDebugText(Pint& pint, GLFontRenderer& renderer, float y, float text_scale)
	{
		...
		if(!pint.GetD6Info(Handle, Info))
			return y;
		renderer.print(0.0f, y, text_scale, _F("Swing Y angle: %.5f\n", Info.mSwingYAngle * RADTODEG + APIRotY));	y -= text_scale;
		renderer.print(0.0f, y, text_scale, _F("Swing Z angle: %.5f\n", Info.mSwingZAngle * RADTODEG + APIRotZ));	y -= text_scale;
		return y;
	}
