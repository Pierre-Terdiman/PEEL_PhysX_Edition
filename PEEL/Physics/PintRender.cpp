...
//	mCurrentShape	(null),
	...
	mRenderEnabled	(true),
	mSelected		(false),
	mHasFocusActor	(false),
	mHasFocusShape	(false),
...
void DefaultRenderer::EndRender()
{
	...
	DrawSelectedShapes(mSelectedShapes, Point(1.0f, 0.0f, 0.0f));
...
bool DefaultRenderer::SetCurrentActor(PintActorHandle h)
{
	...
	if(RenderEnabled && mSelManager)
		mSelected = mSelManager->IsSelectedNative(mPint, h);
	else
		mSelected = false;
	mRenderEnabled = RenderEnabled;
	//mCurrentActor = h;
	mHasFocusActor = h==mFocusActor;
...
void DefaultRenderer::DrawShape(PintShapeRenderer* shape, const PR& pose)
{
	...
	if(1 && mSelected)
	{
		Container& Storage = (mHasFocusActor || mHasFocusShape) ? mFocusShapes : mSelectedShapes;
