...
//	#pragma comment(lib, "../../Ice/Lib64/IML64.lib")
	...
	#ifdef _DEBUG
		#pragma comment(lib, "../../../../PEEL_Externals/Jolt_5_5_0/Lib/x64/Debug/Jolt.lib")
	#else
		#ifdef JPH_USE_AVX
			#pragma comment(lib, "../../../../PEEL_Externals/Jolt_5_5_0/Lib/x64/Distribution/Jolt.lib")
		#else
			#pragma comment(lib, "../../../../PEEL_Externals/Jolt_5_5_0/Lib/x64/Release/Jolt.lib")
		#endif
	#endif
#else
	...
	virtual void					Free(void *inAddress, uint inSize) override
	{
		...
		else
		{
			...
			if (mBase + mTop != inAddress)
				ASSERT(0); // Freeing in the wrong order
		}
	}
...
static void* reallocate(void *inBlock, size_t inOldSize, size_t inNewSize)
{
	void* newData = AllocateHook(inNewSize);
	if(inBlock)
	{
		const size_t copySize = inNewSize > inOldSize ? inOldSize : inNewSize;
		CopyMemory(newData, inBlock, copySize);
		FreeHook(inBlock);
	}
	return newData;
}
...
void JoltPint::Init(const PINT_WORLD_CREATE& desc)
{
	...
	// PT: I can't seem to be able to implement reallocate properly so for now, switch to default allocators
	...
	gPhysicsSystem->Init(gMaxBodies, gNbBodyMutexes, gMaxBodyPairs, gMaxContactConstraints, gBroadPhaseLayerInterface, gObjectVsBroadPhseLayerFilter, gObjectLayerPairFilter);
...
static inline_ void FillResultStruct(const RRayCast& raycast, PintRaycastHit& hit, const RayCastResult& result)
{
...
udword JoltPint::BatchRaycastAny(PintSQThreadContext context, udword nb, PintBooleanHit* dest, const PintRaycastData* raycasts)
{
	...
	RayCastSettings inRayCastSettings;
	inRayCastSettings.SetBackFaceMode(gBackfaceCulling ? EBackFaceMode::IgnoreBackFaces : EBackFaceMode::CollideWithBackFaces);
...
udword JoltPint::BatchBoxSweeps(PintSQThreadContext context, udword nb, PintRaycastHit* dest, const PintBoxSweepData* sweeps)
{
	...
	while(nb--)
	{
		...
		const RShapeCast shape_cast { &QueryShape, Vec3::sReplicate(1.0f), Mat44(v0,v1,v2,v3), ToVec3(sweeps->mDir * MaxDist) };

		ClosestHitCollisionCollector<CastShapeCollector> collector;
		NPQ.CastShape(shape_cast, settings, RVec3::sZero(), collector);
...
udword JoltPint::BatchSphereSweeps(PintSQThreadContext context, udword nb, PintRaycastHit* dest, const PintSphereSweepData* sweeps)
{
	...
	while(nb--)
	{
		...
		const RShapeCast shape_cast { &QueryShape, Vec3::sReplicate(1.0f), Mat44::sTranslation(ToVec3(sweeps->mSphere.mCenter)), ToVec3(sweeps->mDir * MaxDist) };
...
udword JoltPint::BatchCapsuleSweeps(PintSQThreadContext context, udword nb, PintRaycastHit* dest, const PintCapsuleSweepData* sweeps)
{
	...
	while(nb--)
	{
		...
		const RShapeCast shape_cast { &QueryShape, Vec3::sReplicate(1.0f), Mat44::sRotationTranslation(ToJQuat(q), ToVec3(Center)), ToVec3(sweeps->mDir * MaxDist) };
...
udword JoltPint::BatchSphereOverlapAny(PintSQThreadContext context, udword nb, PintBooleanHit* dest, const PintSphereOverlapData* overlaps)
{
	...
	while(nb--)
	{
		...
		NPQ.CollideShape(&QueryShape, Vec3::sReplicate(1.0f), Mat44::sTranslation(ToVec3(overlaps->mSphere.mCenter)), settings, RVec3::sZero(), collector);
...
class JoltPlugIn : public PintPlugin
{
	...
	virtual	const char*	GetTestGUIName()																				override	{ return "Jolt 5.5.0";	}
